...
- [ ] 补全 `_build_authorization` 签名逻辑，符合官方 README（canonical request + Txn Hash）。
- [ ] 配置加载：支持 `GREENFIELD_SP_HOST`、`GREENFIELD_BUCKET`、`GREENFIELD_PRIVATE_KEY`、`GREENFIELD_TXN_HASH`、可选 content-type。
- [ ] 单元测试：
  - [ ] 使用 requests-mock/monkeypatch，验证 URL 形态为 `https://{bucket}.{sp_host}/{object}`，Header 带 `X-Gnfd-Txn-Hash`、Authorization 被调用。
  - [ ] `_gen_key` 为空 key 时生成非空键。

Phase 3：后端切换与文档/配置验证
- [ ] 工厂按 `REPUTATION_BACKEND` 返回 IPFS 或 Greenfield，默认 IPFS。
- [ ] 更新环境变量示例与 README/配置文档（Plan 已含，必要时同步 SDK README）。
- [ ] 预留降级策略：Greenfield 异常时清晰日志，不影响 IPFS 路径。
- [ ] 单元测试：
  - [ ] 参数化 `REPUTATION_BACKEND` 为 ipfs/greenfield，验证工厂实例类型。
  - [ ] 缺失必要 Greenfield 配置时抛出明确异常或日志警告。
